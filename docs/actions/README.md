...
```shell
python3 -m pip install --upgrade pip wheel setuptools
python3 -m pip install --upgrade acryl-datahub-workflows
datahub workflows version
```
...
options: 
  ...
  failed_events_dir: "/tmp/datahub/workflows"  # The directory in which to write a failed_events.log file that tracks events which fail to be processed. Defaults to "/tmp/logs/datahub/workflows". 
