  ...
  create  table
    "production"."intermediate"."int_product__degree_popular_documents__dbt_tmp"
...
degree_document_rank AS (
    SELECT
        document_id,
        degree_id,
        ROW_NUMBER() OVER (PARTITION BY degree_id ORDER BY l30d_laplace_score DESC, overall_laplace_score DESC, rating_positive DESC, rating_negative ASC, document_id DESC) as document_rank
    FROM
        document_laplace
)
SELECT
