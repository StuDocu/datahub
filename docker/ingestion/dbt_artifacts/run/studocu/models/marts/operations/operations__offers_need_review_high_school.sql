  ...
  create  table
    "production"."operations"."offers_need_review_high_school__dbt_tmp"
...
select * from source
        ...
        select
            country_id,
            max(high_school_paid_acquisition)::int::bool as is_country_open_to_purchase
        from (
            select
                country_id,
                high_school_paid_acquisition::int
            from "production"."production_studocu"."degree" 
            group by 1, 2
            ...
            select
                i.country_id,
                r.high_school_paid_acquisition
            from "production"."production_studocu"."institution" i
            left join "production"."production_studocu"."region" r
                on i.region_code = r.code
            group by 1, 2) 
        group by 1
        ...
        left join user_offer_institution as uoi
            on uoi.user_id = ab.user_id
        group by 1, 2, 3
    ), degree_offer_data as (
        SELECT
            eab.user_id,
            listagg(distinct dgr.id, ',') as offer_degree_ids,
            listagg(distinct dgr.name, ',') as offer_degree_names,
            listagg(distinct c.grade_id, ',') as offer_grade_ids,
            listagg(distinct g.name, ',') as offer_grade_names
        FROM eligible_acquisition_batch as eab
        join "production"."production_studocu"."document_acquisition_batch" as dab
            on eab.id = dab.acquisition_batch_id
        join "production"."production_studocu"."document" as d
            on d.id = dab.document_id
        join "production"."production_studocu"."course" as c
            on c.id = d.course_id
        left join "production"."production_studocu"."grade" as g
            on c.grade_id = g.id
        left join "production"."production_studocu"."degree" as dgr
            on dgr.id = coalesce(c.degree_id,g.degree_id)
        group by 1
    )
    select
      u.*,
      ...
      dod.offer_degree_ids,
      dod.offer_degree_names,
      dod.offer_grade_ids,
      dod.offer_grade_names,
      GETDATE() as datamart_updated_at
    from __dbt__cte__operations__offers_need_review_base as u
    join acquisition_batch_data as abd
    ...
    left join degree_offer_data as dod
        on dod.user_id = u.user_id
    where
        exists (
            select 1
